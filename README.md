...
**状态：🚧 M1.3 — Doubao 适配器（Volc bigmodel 二进制协议）已落地，daemon 三种 provider 都可切换**
...
```sh
# 1. 零配置 demo (Mock provider)
cargo run -p synapse-daemon --bin synapsed

# 2. 任意符合 Synapse Streaming ASR v0 JSON 协议的 WS 后端
SYNAPSE_ASR_ENDPOINT=wss://your-asr.example.com/v0 \
SYNAPSE_ASR_TOKEN=...                              \
cargo run -p synapse-daemon --bin synapsed

# 3. 豆包 / Volc bigmodel 流式 ASR
cp .env.example .env                # 然后填入真 ARK key
set -a; source .env; set +a
cargo run -p synapse-daemon --bin synapsed
# → asr_provider=doubao
```

> ⚠️ **API key 安全**：`.env` 已 gitignore。永远不要把 `ark-...` 形态的 key 写进任何 commit、issue、PR 描述、聊天记录。一旦泄露立刻去 [ARK 控制台](https://console.volcengine.com/ark) 旋转。
